...
서비스 계약은 `packages/core/src/{llm,session,generation}.ts`에 정의하고, 구체 구현은 플러그인으로 주입합니다. AG-UI 타입, 이벤트 상수, 검증 기능은 `@keundal/core`에서 가져옵니다. `@ag-ui/core` 직접 의존은 코어의 프로토콜 래퍼에서만 관리합니다.
