...
플러그인 설정과 사용법은 [에이전트 문서](packages/plugin-agent-simple/README.md), [OpenAI 문서](packages/plugin-llm-openai/README.md), [Gemini 문서](packages/plugin-llm-gemini/README.md), [Anthropic 문서](packages/plugin-llm-anthropic/README.md), [메모리 저장소 문서](packages/plugin-store-memory/README.md), [IndexedDB 저장소 문서](packages/plugin-store-indexeddb/README.md), [SQLite 저장소 문서](packages/plugin-store-sqlite/README.md)를 참고합니다.
...
- `generation.run()`은 `GenerationOptions.stream`이 있으면 해당 이벤트 공급자에 취소 신호를 전달하여 실행합니다. 함수는 실행 요청과 분리하며 저장하거나 복구 시 재호출하지 않습니다.
...
에이전트의 `tools` 설정은 도구 정의와 실행 함수를 함께 받습니다. 도구 호출을 수집하고 실행 결과를 후속 요청에 넣는 전체 과정은 하나의 generation 실행으로 기록합니다. 후속 요청에도 입력 예산 검사를 적용하며, `maxToolRounds`의 기본값은 8입니다. 도구를 등록하지 않으면 기존의 단일 생성 동작을 유지합니다.
