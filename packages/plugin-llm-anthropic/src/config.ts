...
export function parseConfig(value: unknown): AnthropicLLMConfig {
...
export const AnthropicLLMConfigSchema: StandardSchemaV1<AnthropicLLMConfig, AnthropicLLMConfig> = {
  '~standard': {
    version: 1,
    vendor: 'keundal',
    validate(value) {
      try {
        ...
        return { issues: [{ message: error instanceof Error ? error.message : 'invalid Anthropic configuration' }] }
      }
    }
  }
}
